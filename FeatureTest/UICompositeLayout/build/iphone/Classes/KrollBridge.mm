...
@implementation UICompositeLayoutObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_uicompositelayout);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any UICompositeLayout patches in JS before a context is loaded... nice for 
...
{
	// create UICompositeLayout global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_uicompositelayout = [[UICompositeLayoutObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_uicompositelayout];
	
	NSString *_uicompositelayoutNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _uicompositelayoutNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_uicompositelayout valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.uicompositelayout.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
